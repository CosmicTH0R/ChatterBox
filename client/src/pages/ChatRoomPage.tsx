          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              <Transition.Child
                ...
                leaveTo="opacity-0 scale-95"
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-xl bg-[var(--dc-bg-tertiary)] p-6 text-left align-middle shadow-2xl transition-all border border-[var(--dc-border)]">
                  <Dialog.Title
                    as="h3"
                    className="text-lg font-bold text-[var(--dc-text-white)] flex items-center gap-2"
                  >
                    <Trash2 className="w-5 h-5 text-[var(--dc-danger)]" /> Confirm Unsend
                  </Dialog.Title>
                  <div className="mt-4">
                    <p className="text-[var(--dc-text-muted)] text-[14px]">
                      Are you sure you want to unsend <span className="text-[var(--dc-text-white)] font-bold">ALL</span> of your messages
                      in this room? This action is permanent and cannot be undone.
                    </p>
                  </div>
                  ...
                  <div className="mt-6 flex justify-end gap-3">
                    <button
                      type="button"
                      className="px-4 py-2 rounded text-sm font-medium text-[var(--dc-text-normal)] hover:bg-[var(--dc-bg-active)] transition-colors"
                      onClick={() => setIsUnsendMyModalOpen(false)}
                    >
                      Cancel
                    </button>
                    <button
                      type="button"
                      className="px-4 py-2 rounded text-sm font-medium text-white bg-[var(--dc-danger)] hover:opacity-90 transition-all shadow-lg shadow-[var(--dc-danger)]/10"
                      onClick={handleConfirmUnsendMyMessages}
                    >
                      Unsend All
                    </button>
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-xl bg-[var(--dc-bg-tertiary)] p-6 text-left align-middle shadow-2xl transition-all border border-[var(--dc-border)]">
                  <Dialog.Title
                    ...
                    className="text-lg font-bold text-[var(--dc-text-white)] flex items-center gap-2"
                  >
                    <X className="w-5 h-5 text-[var(--dc-danger)]" /> Clear Chat History
                  </Dialog.Title>
                  <div className="mt-4">
                    <p className="text-[var(--dc-text-muted)] text-[14px]">
                      As the creator, are you sure you want to <span className="text-[var(--dc-text-white)] font-bold">clear all messages</span> in this room for everyone?
                      This action is permanent.
                    </p>
                  </div>
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-xl bg-[var(--dc-bg-tertiary)] p-6 text-left align-middle shadow-2xl transition-all border border-[var(--dc-border)]">
                  <Dialog.Title
                    ...
                    className="text-lg font-bold text-[var(--dc-text-white)] flex items-center gap-2"
                  >
                    <Edit className="w-5 h-5 text-[var(--dc-accent)]" /> Edit Message
                  </Dialog.Title>
                  <form onSubmit={handleEditSubmit} className="mt-4">
                    <textarea
                      value={editedText}
                      onChange={(e) => setEditedText(e.target.value)}
                      rows={4}
                      className="w-full p-3 bg-[var(--dc-bg-primary)] border border-[var(--dc-border)] rounded-lg text-[var(--dc-text-normal)] focus:outline-none focus:border-[var(--dc-accent)] transition-colors resize-none"
                      placeholder="Edit your message..."
                      autoFocus
                    />
                    {messageToEdit?.fileUrl && (
                      <p className="text-xs text-[var(--dc-text-muted)] mt-2">
                        Note: Media file ({messageToEdit.fileType}) cannot be changed.
                      </p>
                    )}
                    <div className="mt-6 flex justify-end gap-3">
                      ...
                      <button
                        type="submit"
                        className="px-4 py-2 rounded text-sm font-medium text-white bg-[var(--dc-accent)] hover:bg-[var(--dc-accent-hover)] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-[var(--dc-accent)]/10"
                        disabled={!editedText.trim() && !messageToEdit?.fileUrl}
                      >
                        Save Changes
                      </button>
